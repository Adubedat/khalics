...
class HorizontalPicker extends Component {
  constructor(props) {
    ...
    renderChildren = children => children.map(this.renderChild)
    ...
    renderChild = (child) => {
      ...
      const fontSize = itemValue !== this.props.selectedValue ? 16 : 22;
      return (
        <TouchableWithoutFeedback key={itemValue} onPress={this.handleItemPress(itemValue)}>
          <View style={[styles.itemContainer, { width: this.getItemWidth() }]}>
            <Text style={[styles.itemText, child.props.style, { color, opacity, fontSize }]}>{child.props.label}</Text>
          </View>
        </TouchableWithoutFeedback>
      );
    }
